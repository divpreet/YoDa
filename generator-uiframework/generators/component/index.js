...
var AngularDirectiveGenerator = yeoman.generators.Base.extend({
   ...
   askFactory: function () {
      ...
      this.prompt({
         type: 'input',
         name: 'factoryNeeded',
         message: 'Do you want to create a factory as well?',
         default: 'Y'
      }, function (result) {
         var needFactory = result.factoryNeeded;
         if(needFactory && (needFactory.toUpperCase() === 'Y' || needFactory.toUpperCase() === 'YES' || needFactory.toUpperCase() === 'TRUE')){
            this.composeWith('uiframework:factory', {
               options: {
